...
- 邮箱和日历要用的密码：
  ...
  - Gmail / QQ / 163 的服务器设置按各家公开文档预置，目前实测过的是 iCloud 和 Outlook；遇到问题可以选「其他邮箱」手动填。
  - Outlook / Hotmail / Microsoft 365：不用密码，用微软账号授权登录，见下面「Outlook」一节。
...
2. **邮箱** — 选 iCloud / Outlook / Gmail / QQ / 163 邮箱 / 其他 IMAP 服务器，粘贴密码（Outlook 是授权登录）。
...
| `mailminder account [add\|remove\|password\|folders]` | 管理邮箱账号、要读的文件夹、App 专用密码（Outlook 用 `password` 重新授权）；不带参数列出已有账号 |
...
微软从 2024 年 9 月 16 日起不再接受用密码（包括应用密码）登录 Outlook.com / Hotmail / Live 的 IMAP，Microsoft 365 更早就关了。所以 Outlook 邮箱走微软账号授权：

- 向导里会显示一个网址（https://microsoft.com/devicelogin）和一串代码（15 分钟内有效）。在任意设备上（手机也行）打开网址、输入代码，用那个邮箱的账号登录并同意授权，向导会自动继续。
- 如果你的微软账号是用别家邮箱（比如 Gmail）注册的，登录名和 Outlook 邮箱地址不一样；向导会再问一次 Outlook 邮箱本身的地址（@outlook.com / @hotmail.com 那个）。
- Mailminder 只申请「读邮件（IMAP）」和「保持登录」两项授权；钥匙串里存的是微软发的授权令牌，不是你的密码。每次运行用它换一个一小时有效的访问令牌。
- 授权失效（比如你在微软账号里撤销了它）时，后台任务会失败，连续失败会在日历上提醒你；运行 `mailminder account password` 重新授权即可。

微软只给注册过的应用发令牌。Mailminder 自带一个注册好的应用（授权页上显示名字 Mailminder，并标着「未验证」——没做微软的发布者认证，不影响使用），直接登录即可。

想改用自己注册的应用：按下面的步骤注册（免费，约 5 分钟），把 `config.toml` 里这个账户的 `client_id` 换成你的 ID，再运行 `mailminder account password` 重新授权。

1. 打开 https://entra.microsoft.com 用微软账号登录。个人账号如果提示没有目录，先到 https://azure.microsoft.com/free 开一个免费 Azure 账户（需要手机号和信用卡验证身份，不扣费）。
2. 「应用注册」→「新注册」：名称填 `Mailminder`；支持的账户类型选「任何组织目录中的帐户和个人 Microsoft 帐户」；重定向 URI 不用填（设备码登录用不到）。
3. 注册后进「身份验证」，把「允许公共客户端流」设为「是」，保存。
4. 在「概述」页复制「应用程序(客户端) ID」。应用 ID 不是密码，不需要保密。

另外：
- 个人邮箱要确认 Outlook 网页版 设置 → 邮件 → 转发和 IMAP →「允许设备和应用使用 IMAP」是打开的。
- 公司/学校的 Microsoft 365 邮箱：管理员可能禁止员工授权未经验证的应用，或者关了 IMAP。遇到「需要管理员同意」或登录被拒，要找 IT。
...
- 把 Google 日历或 Outlook 日历当写入目标：请用 iCloud 日历（或其他支持 CalDAV 的日历）。
